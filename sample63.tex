...
% \textcolor{red}{this paragraph should find a better place as suggested in the above}
...
\subsection{Source detection}\label{longT}
...
\caption{2--8 keV counts image of the Limiting Window, combining 13 {\it Chandra}/ACIS-I observations. Locations of the 24 periodic sources are marked with circles ({\it magenta}: 10 sources previously found by \cite{2012ApJ...746..165H}; {\it red}: 12 newly discovered in this work and belonging to the Galactic bulge; {\it green}: two newly discovered and located in the foreground) and numbered as in Table~\ref{tab:src}.}
\label{fig:FoV}
\end{figure*}

\section{Period Finding Algorithm}\label{sec:methods}
The period-finding algorithms can be divided into three groups. 
\\
\indent
The most traditional algorithms are based on Fourier transform and its power density spectra, which includes the classical methods named Schuster periodogram \citep{1898TeMag...3...13S}, the fourier analysis with unequally-Spaced Data \citep{1975Ap&SS..36..137D}, the correlation-based method \citep{1988ApJ...333..646E} and so on.
\\
\indent
Another method is called the least square method, which fits the data with a model in frequency space. Using statistics like least-squares residuals to define the likelihood function and then selecting the frequency which maximizes the likelihood. The famous periodogram called Lomb-Scargle \citep[hereafter LS]{1976Ap&SS..39..447L,1982ApJ...263..835S} also belongs to this category. Note that when choosing trigonometric functions, the least-squares method always falls into Fourier method. Another track is to replace the least-squares residuals with polynomial fits, such as \citep{1996ApJ...460L.107S}.
\\
\indent
The last group is phase-folding methods. The procedure is described as follows. Firstly, choose a trial period. Secondly, fold data set from observations as a function of phase. Thirdly, optimize the cost function through frequency space. Methods falling in this category use diverse cost functions. Several widely known examples are Epoch Folding (EF) method \citep{1983ApJ...266..160L}, the Phase Dispersion Minimization \citep{1978ApJ...224..953S}, the Gregory-Loredo (GL) method \citep{1992ApJ...398..146G}.
\\
\indent
...
\section{X-ray timing analysis}\label{sec:timing}
\subsection{Periodic variability}\label{periodic}
For the sake of changing aimpoint, we used individual source extract region for different observations.  Note that we do the time correction for each observation by applying the CIAO tool \emph {axbary} in advance. Explicitly, the 90\% enclosed counts radius(ECR) aperture  whose radius defined by specific PSF map was applied for counts in that observation. The combination of each event list was then employed as the input of GL method. Since the phase-folding method directly manipulated each photons and we could not distinguish background photon from source photon, the local background information were not provided for timing analysis. 
...
Besides, in order to compensate for uneven distribution over the bins, we defined "epoch" for each source. The "epoch" offered the start time and end time of each observations covered the source with at least one counts in source region. See \ref{A17} in appendix for detailed illustration about the use of epoch.
\\
\indent
The algorithm employed equidistant trapezoidal integral and parallel computing, which demanded the great computational power. Thus the frequency resolution was chosen by balancing the running time and the accuracy of the algorithm. 
\\
\indent
...
The total span of 13 observation is three years, i.e., $10^8s$, so the resolution of angular frequency is about $5\times 10^{-9}$. When the $\omega$ increases, corresponding the period decreases, the same resolution brings numerous discrete frequencies and raises running time greatly. Thus we used three different resolutions $10^{-7},10^{-8},10^{-9}$ in three period ranges $(300s,3000s),(3000s,10000s), (10000s,50000s)$. The ranges was chosen for the orbital period distribution of CVs. There is a period gap in distribution at 2-3 hours, and a period minimum at about 70 minutes. Then the first range covers roughly the spin period of IPs, the other two ranges overlay the orbital period below and beyond the period gap respectively.
\\
\indent
As described in the appendix, we handled the data under circumstance of period and phase are both unknown. After the processing, the analysis for each source produced a probability of periodic and the most probable angular frequency corresponding the period of signal. In this work, we took probability of 90\% as threshold to determine the periodicity. 

\subsection{Detection completeness}\label{simulation}
...
\section{Period finding results}\label{sec:results}
