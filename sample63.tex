...
We thus simulate 100 light curves for each combination of parameters, taking into account Poisson errors and the exact exposures of the 13 ACIS-I observations. We then search for a period from the simulated light curve using the GL algorithm in the same manner as in Section~\ref{subsec:appli}.
...
We processed the EPIC PN and MOS data with the standard tool \emph{emchain} and \emph{epchain} in SAS v18.0.0. Then we extract the 0.2-10 keV light curve and folded it from PN data for it has the largest effective area.
